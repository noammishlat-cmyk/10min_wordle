...
export default function Home() {
  ...
  const [modalTitle, setModalTitle] = useState("ניחשת את המילה בהצלחה!");
  const [wordWasTitle, setWordWasTitle] = useState("המילה הנכונה היא ");
  ...
  const onKeyAction = (key: string) => {
    ...
    if (/^[א-ת]$/.test(key) && currentGuess.length < 5) {
      const nextGuess = currentGuess + key;
      setCurrentGuess(nextGuess);
      const newGuesses = [...guesses];
      newGuesses[currentLine][nextGuess.length - 1] = key;
      setGuesses(newGuesses);
    }
  };
  ...
  if (!isLoaded || !currentWordIndex) return;
    ...
    Cookies.set('save_data_heb', JSON.stringify(dataToSave), { expires: 1 });
  }, [guesses, guessesPlace, currentLine, stopKeyboard, isLoaded, currentWordIndex]);

  useEffect(() => {
    const initializeGame = async () => {
      // 1. Get the TRUTH from the server first
      ...
      // 2. Check the Cookie
      const saved = Cookies.get('save_data_heb');
      ...
      if (saved) {
        try {
          ...
          // Use the raw data from the cookie, NOT the state variable
          if (parsed.currentWordIndex && serverWordIndex !== parsed.currentWordIndex) {
            console.log("Word changed! Clearing board...");
            Cookies.remove('save_data_heb');
            // We don't need to reload, just let the states stay at default
            setCurrentWordIndex(serverWordIndex); 
            setIsLoaded(true);
            return; 
          }

          // 4. If indices match, load the data
        ...
        // No cookie? Just set the current index
        setCurrentWordIndex(serverWordIndex);
      }
  ...
  const checkWordOnBackend = async (guess: string) => {
    try {
      const response = await fetch(`${apiUrl}/check_heb?word=${guess}`);
      ...
      if (data.isValid) {
        ...
        if (is_correct){
          setShowModal(true);
          setModalTitle("ניחשת את המילה בהצלחה!");
          setWordWasTitle("המתן למילה הבאה");
  ...
  useEffect(() => {
    const handleGameOver = async () => {
      ...
      if (currentLine > 4) {
        setModalTitle("לא הצלחת הפעם...");
        setShowModal(true);
        setStopKeyboard(true);

        try {
          const response = await fetch(`${apiUrl}/correct_heb`);
          const data = await response.json();

          const correct = data.correct;
          setWordWasTitle(`המילה הנכונה היא ${correct}`);

        } catch (error) {
          console.error("Error fetching the word:", error);
          setWordWasTitle("בעיה בטעינת המילה");
        }
      }
    };
  ...
  const HEBREW_MAP: Record<string, string> = {
    KeyA: 'ש', KeyB: 'נ', KeyC: 'ב', KeyD: 'ג', KeyE: 'ק',
    KeyF: 'כ', KeyG: 'ע', KeyH: 'י', KeyI: 'ן', KeyJ: 'ח',
    KeyK: 'ל', KeyL: 'ך', KeyM: 'צ', KeyN: 'מ', KeyO: 'ם',
    KeyP: 'פ', KeyQ: '/,', KeyR: 'ר', KeyS: 'ד', KeyT: 'א',
    KeyU: 'ו', KeyV: 'ה', KeyW: 'original_w', KeyX: 'ס', KeyY: 'ט',
    KeyZ: 'ז', Semicolon: 'ף', Quote: 'ת', Comma: 'ת', Period: 'ץ', Slash: '.'
  };
  ...
  <div className="flex flex-col min-h-screen bg-black font-sans text-white items-center p-4">
    ...
    <main className="grow flex flex-col items-center justify-center w-full max-w-2xl bg-zinc-900/50 rounded-[40px] p-6 md:p-12 shadow-2xl border border-white/5">
      <h3 className="text-xl md:text-2xl font-bold mb-6 text-zinc-300">נחש את המילה</h3>
      
      <div dir="rtl" className="flex flex-col gap-2 md:gap-4">
          ...
          {row.map((key) => {
            ...
            return (
              <button
                ...
                className={`
                  ${key === 'ENTER' || key === '⌫' ? 'px-4 text-[10px] min-w-12' : 'flex-1 text-sm md:text-lg'}
    ...
    <p className="text-center text-[10px] text-zinc-500 mt-4 uppercase tracking-widest">
      © (Nomspot) נוצר על ידי נעם גרינברג
    </p>
    <AnimatePresence>
      {showModal && (
        <motion.div dir="rtl" className="fixed inset-0 z-50 flex items-center justify-center bg-white/30 backdrop-blur-sm p-12"
